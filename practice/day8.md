...
### 1. 조건 기반 파생 컬럼 — `np.where` ★★★

```python
import numpy as np

df['grade'] = np.where(df['score'] >= 60, '합격', '불합격')
...
> 읽는 법: "90 이상이면 A, 아니면 (70 이상이면 B, 아니면 C)"
...
> `bins` — 경계값 리스트 (N개 구간 → N+1개 경계값)
> 기본적으로 왼쪽 열린 구간 `(0, 30]` — 30은 포함, 0은 미포함
> `groupby` 시 `observed=True` 붙이면 FutureWarning 제거
...
> 딕셔너리로 값을 일대일 변환. 매핑에 없는 값은 NaN.

---

### 4. 함수 적용 — `apply` + `lambda`

```python
df['salary_man'] = df['salary'].apply(lambda x: x / 10000)
df['pass'] = df['score'].apply(lambda x: '합격' if x >= 60 else '불합격')
```

> `lambda x: 표현식` — x를 받아서 표현식 반환. 간단한 함수를 한 줄로.
> 간단한 조건은 `np.where`가 더 빠름. `apply`는 복잡한 로직에 사용.
...
> `index` — 행, `columns` — 열, `values` — 집계할 값, `aggfunc` — 집계 함수
> `A × B` 표현 → `index=A`, `columns=B`
> groupby와 결과 동일하지만 교차표 형태로 출력
...
```python
print(df['score'].corr(df['salary']))           # 두 컬럼 상관계수 값
print(df.corr(numeric_only=True))               # 전체 수치 컬럼 상관계수 행렬
print(df[['age', 'salary']].corr())             # 특정 컬럼들만
```

> -1 ~ 1 사이 값. 1에 가까울수록 강한 양의 상관, -1에 가까울수록 강한 음의 상관
